...
{
    if (!radio || radio->hal_context != &s_radio_context)
    {
        return NULL;
    }
...
sx127x_esp_err_t sx127x_esp_init(sx127x_t *radio, const sx127x_esp_config_t *config)
{
    ...
    /* F3: Create RX timer unconditionally (eager, not lazy) */
    err = sx127x_esp_create_rx_timer(&s_radio_context);
    if (err != SX127X_ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to create RX timeout timer during init");
        return err;
    }
    ...
    if (!s_radio_context.spi_mutex || !s_radio_context.state_mutex)
    {
        ...
        if (s_radio_context.spi_mutex)
            vSemaphoreDelete(s_radio_context.spi_mutex);
        if (s_radio_context.state_mutex)
            vSemaphoreDelete(s_radio_context.state_mutex);
    ...
    // Set radio context
    radio->hal_context = &s_radio_context;
...
sx127x_esp_err_t sx127x_esp_deinit(sx127x_t *radio)
{
    ...
    /* F1: Delete RX timer BEFORE queue/task to prevent callback racing with deinit */
    if (ctx->rx_timer)
    {
        /* Set deiniting flag under lock so callback can bail */
        portENTER_CRITICAL(&ctx->rx_timer_lock);
        ctx->rx_timer_deiniting = true;
...
/**
 * @brief Initialize radio for LBM modem use
 */
sx127x_esp_err_t sx127x_esp_init_for_lbm(sx127x_t *radio)
{
    ...
    // Initialize the ESP32 SX127x wrapper
    sx127x_esp_err_t err = sx127x_esp_init(radio, &config);
    if (err != SX127X_ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to initialize SX127x for LBM: %s", sx127x_esp_err_to_string(err));
        return err;
    }

    ESP_LOGI(TAG, "SX127x initialized for LBM successfully");
    return SX127X_ESP_OK;
}
...
const char *sx127x_esp_err_to_string(sx127x_esp_err_t err)
{
    ...
    case SX127X_ESP_OK:
        return "Success";
    case SX127X_ESP_ERR_INVALID_ARG:
        return "Invalid argument";
    case SX127X_ESP_ERR_NO_MEM:
        return "Out of memory";
    case SX127X_ESP_ERR_SPI_INIT:
        return "SPI initialization failed";
    case SX127X_ESP_ERR_GPIO_INIT:
        return "GPIO initialization failed";
    case SX127X_ESP_ERR_INTERRUPT_INIT:
        return "Interrupt initialization failed";
    case SX127X_ESP_ERR_RADIO_NOT_FOUND:
        return "Radio not responding";
    case SX127X_ESP_ERR_TIMEOUT:
        return "Operation timeout";
    case SX127X_ESP_ERR_NOT_INITIALIZED:
        return "Component not initialized";
    case SX127X_ESP_ERR_RADIO_BUSY:
...
/**
 * @brief Handle synthetic RX timer event (F4: extracted to reduce event_task CCN)
 */
static void sx127x_esp_handle_rx_timer_event(sx127x_esp_context_t *ctx, const sx127x_esp_dio_event_t *event)
{
...
void sx127x_esp_event_task(void *pvParameters)
{
    ...
    while (1)
    {
        if (xQueueReceive(ctx->event_queue, &event, portMAX_DELAY))
        {
            ...
            /* F10: Distinguish synthetic RX timer event from DIO interrupts, use PRIu64 */
            if (event.dio_num == SX127X_ESP_EVENT_RX_TIMER)
            {
                ESP_LOGD(TAG, "Processing RX timeout event (synthetic) at %" PRIu64 " us", event.timestamp);
            }
            else
            {
                ESP_LOGD(TAG, "Processing DIO%d interrupt at %" PRIu64 " us", event.dio_num, event.timestamp);
            }
#endif
            ...
            /* Handle synthetic RX timeout event (F4: extracted to reduce CCN) */
            if (event.dio_num == SX127X_ESP_EVENT_RX_TIMER)
            {
                sx127x_esp_handle_rx_timer_event(ctx, &event);
            }
